...
def make_box(point, rad):
	box     = np.array([point - rad, point + rad])
	box[:,1]= box[::-1,1] # reverse ra
	return box

def filter_map(map, lknee=3000, alpha=-3, beam=0):
	fmap  = enmap.fft(map)
...
for fi in range(comm.rank, len(ifiles), comm.size):
	...
	if not in_box(info.box, ast_pos0):
		if verbose >= 3: print(colors.lgray + message + " outside" + colors.reset)
		continue
	...
	if np.all(tmap == 0):
		if verbose >= 2: print(colors.white + message + " unhit" + colors.reset)
		continue
	...
	if err > time_tol or abs(ctime-ctime0) > time_sane:
		if verbose >= 2: print(colors.white + message + " time" + colors.reset)
		continue
	...
	if verbose >= 1: print(colors.lgreen + message + " ok" + colors.reset)
